...
import { ChevronDown, ChevronLeft, ChevronRight, Filter, Plus, Tags, BookOpen, Calendar as CalendarIcon, Clock, X, Edit, Trash2, Search, Settings, RefreshCw, List, GlobeLock } from 'lucide-react';
...
function CalendarPage() {
    ...
    const renderMonthView = () => {
        ...
        for (let i = 0; i < startingDayOfWeek; i++) {
            days.push(
                <div key={`empty-${i}`} className="min-h-24 lg:min-h-32 bg-muted/70 border border-border"></div>
            );
        }
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            days.push(
                <div
                    key={day}
                    className="min-h-24 lg:min-h-32 bg-card border border-border p-2 hover:bg-muted/50 transition-colors cursor-pointer overflow-hidden"
                    onClick={() => {
                        setCurrentDate(date);
                        setViewMode('day');
                    }}
        ...
        return (
            <div className="bg-card rounded-xl border border-border overflow-x-auto">
                ...
                <div className="grid grid-cols-8 border-b border-border min-w-[800px]">
                    <div className="p-3 lg:p-4 bg-muted/70"></div>
                    {weekDays.map((day, index) => {
                        const isToday = day.toDateString() === new Date().toDateString();
                        return (
                            <div key={index} className="p-3 lg:p-4 text-center border-l border-border bg-muted/70">
                ...
                <div className="grid grid-cols-8 relative min-w-[800px]">
                    ...
                    <div className="bg-muted/70">
                        {timeSlots.map((hour) => (
                            <div key={hour} className="h-16 lg:h-20 border-b border-border p-2 text-xs text-muted-foreground">
                    ...
                    {weekDays.map((day, dayIndex) => (
                        <div key={dayIndex} className="border-l border-border relative">
                            {timeSlots.map((hour) => (
                                <div
                                    key={`${dayIndex}-${hour}`}
                                    className="h-16 lg:h-20 border-b border-border hover:bg-muted/50 cursor-pointer"
                                ></div>
                            ))}
    ...
    const renderDayView = () => {
        ...
        return (
            <div className="bg-card rounded-lg border border-border overflow-hidden">
                {/* Time zone label */}
                <div className="px-4 py-2 bg-muted/70 border-b border-border text-xs text-muted-foreground">
                    GMT-5
                </div>
                ...
                <div className="relative">
                    {hours.map((hour) => (
                        <div key={hour} className="flex border-b border-border">
                            <div className="w-16 lg:w-20 flex-shrink-0 p-2 text-xs text-muted-foreground bg-muted/70">
                                {hour === 0 ? '12 AM' : hour < 12 ? `${hour} AM` : hour === 12 ? '12 PM' : `${hour - 12} PM`}
                            </div>
            ...
            <div className="max-w-7xl mx-auto p-3 lg:p-6">
                ...
                <div className="mb-4 lg:mb-6 flex flex-col lg:flex-row items-stretch lg:items-center gap-3 lg:gap-4">
                    ...
                    <div className="flex items-center gap-2">                        {/* Calendar/Events Toggle */}
                        ...
                        <button className="p-2 bg-card border border-border rounded-lg hover:bg-muted/50">
                            {/* <GoogleIcon /> */}
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            className="p-2 bg-card border border-border rounded-lg hover:bg-muted/50"
                        >
                            ...
                            <div className="bg-card rounded-xl border border-border overflow-hidden">
                                ...
                                <div className="hidden lg:grid grid-cols-7 border-b border-border">
                                    {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
                                        <div key={day} className="p-3 text-center bg-muted/70 border-r border-border last:border-r-0">
                                ...
                                <div className="grid lg:hidden grid-cols-7 border-b border-border">
                                    {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, index) => (
                                        <div key={`${day}-${index}`} className="p-2 text-center bg-muted/70 border-r border-border last:border-r-0">
                                    ...
                                    .map((event) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className={`flex-shrink-0 w-16 h-16 lg:w-20 lg:h-20 rounded-xl flex flex-col overflow-hidden ${isToday ? 'bg-primary/10' : 'bg-muted'
                                                    } border border-border`}>
                                                    <div className={`text-[10px] lg:text-xs font-semibold w-full flex items-center justify-center py-0.5 ${isToday ? 'text-primary' : 'bg-primary text-primary-foreground'
                                                        }`}>
                                                        {monthShort}
                                                    </div>
                                                    <div className={`text-xl lg:text-2xl font-bold leading-none my-1 flex items-center justify-center ${isToday ? 'text-primary' : 'text-foreground'
                                                        }`}>
                                                        {event.date.getDate()}
                                                    </div>
                                                    <div className={`text-[10px] lg:text-xs font-medium flex items-center justify-center ${isToday ? 'text-primary' : 'text-muted-foreground'
                                                        }`}>
                                                        {event.startTime}
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex items-center flex-wrap gap-2 text-xs lg:text-sm text-muted-foreground">
                                                        ...
                                                        <span>•</span><Clock className="w-3 h-3 lg:w-4 lg:h-4" />
